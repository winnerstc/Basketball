...
from pyspark.sql import SparkSession
from pyspark.sql.functions import (
    col, trim, upper, initcap, to_date, current_timestamp, coalesce,
    current_date, datediff, when, input_file_name, row_number, lit, year
)
from pyspark.sql.types import LongType, IntegerType, DoubleType, StringType
from pyspark.sql.window import Window
...
def transform_players_data(df):
    """Clean + Transform the raw bronze players dataframe into Silver + Quarantine."""

    # ------------------------------------------------------------------
    # 1. Normalize fake nulls
    # ------------------------------------------------------------------
    cols_exclude = ["guard", "forward", "center"]
    cols_to_clean = [c for c in df.columns if c not in cols_exclude]

    df_clean = df.replace(["nul", "NULL", "NA", ""], None, subset=cols_to_clean)
    ...
    df_birth = (
        ...
        .withColumn(
            ...
            when(col("birth_date").isNotNull(),
                 col("birth_date").substr(1, 4).cast(IntegerType()))
        )
    )
    ...
    df_flags = (
        df_age
        .withColumn("guard_flag", norm_flag("guard"))
        .withColumn("forward_flag", norm_flag("forward"))
        .withColumn("center_flag", norm_flag("center"))
    )

    # ------------------------------------------------------------------
    # 8. Derive primary_position
    # ------------------------------------------------------------------
    df_pos = (
        df_flags
        .withColumn(
    ...
    # ------------------------------------------------------------------
    # 9. Missing business keys quarantine (personId)
    # ------------------------------------------------------------------
    df_bad_keys = (
        df_pos
        .filter(col("personId").isNull())
    ...
    df_keys_ok = df_pos.filter(col("personId").isNotNull())

    # ------------------------------------------------------------------
    # 10. Numeric rules (basic)
    # ------------------------------------------------------------------
    valid_numeric = (
        (col("height").isNull() | ((col("height") > 10) & (col("height") < 280))) &
        (col("bodyWeight").isNull() | ((col("bodyWeight") > 0) & (col("bodyWeight") < 999)))
    ...
    # ------------------------------------------------------------------
    # 12. Dedup logic
    # Keep newest (birth_date desc, then draftYear desc)
    # ------------------------------------------------------------------
    w = Window.partitionBy("personId").orderBy(
        col("birth_date").desc(),
        col("draftYear").desc_nulls_last()
    )
...
def main(spark, source_db, source_table, silver_path, quarantine_path):
    ...
    df_silver, df_bad = transform_players_data(df)
    # df_silver.write.mode("overwrite").format("parquet").saveAsTable("players_silver")
    df_silver.repartition(10)
    df_silver.write \
        .mode("overwrite") \
        .parquet("hdfs:///tmp/DE011025/NBA/silver/players")
...
if __name__ == "__main__":
    ...
    silver_path = "hdfs:///tmp/DE011025/NBA/silver/players"
    quarantine_path = "hdfs:///tmp/DE011025/NBA/quarantine/players" # Example path
